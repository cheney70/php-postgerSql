 ...
 * PDO PGSQL
 * Class PdoPgsql
 * @package Cheney\Pgsql\Query
 */
class PdoPgsql implements IteratorAggregate
{
     ...
     */
    public function __construct($dbName,$user,$password,$host='127.0.0.1',$port=5432, $primaryKeyName = "id", $foreignKeyName = "%s_id")
    {
    ...
    *-----------------------------------------------------------------------------*/
    ...
    protected function getWhereString()
    {
        ...
        if (!count($this->where_conditions)) {
            return " WHERE ".$this->getPrimaryKeyname()." > 0";
        }
